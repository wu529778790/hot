...
export function parseRelativeDate(date: string, timezone: string = "UTC") {
  ...
  if (matches) {
    ...
    if (lastMatch) {
      ...
      if (beforeMatches && beforeMatches[1]) {
      ...
      if (afterMatches && (afterMatches[1] || afterMatches[2])) {
        const validMatch = afterMatches[1] || afterMatches[2];
        if (validMatch) {
          matches.push(validMatch);
        }
        return dayjs()
